...
Comment développer une application de gestion de comptes clients performante pour répondre aux exigences bancaires modernes ? 

=== 1.4 Objectifs
 
L'application DailyBank doit répondre aux exigeances des clients, pour cela nous avons plusieurs objectifs qui sont : 

** La gestion des compte : nous devons fournir un aperçu clair et complet de tous les comptes, du solde et des transactions récentes. 

** La gestion des transactions : il est primordial de faciliter les transactions de débit ainsi que de crédit.

** Fonctionnalités de virement : Permettre aux utilisateurs d'effectuer facilement des virements entre leurs propres comptes de dépot et autoriser les virements vers des comptes au sein de de la meme institution bancaire.
