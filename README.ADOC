...
| 31/05   | Gantt V1  réalisé    |    link:/S2-05_Gestion_Projet/GantReelV1-PrevV2.pdf[ici] (Telecharger le pdf pour bonne lisibilité)   | Le format de livraison n’est pas respecté (PNG). Il manque le détail de toute la partie dev, on ne retrouve que les activités de GPO (qui sont d’ailleurs incomplète sur la fin du projet). Difficile de comprendre l’enchainement de votre travail. La visualisation dans l’enchainement des taches est trop petit, essayez d’agrandir.
|         | Doc. Util. V1 |   link:/S2-05_Gestion_Projet/Doc-Utilisateur.asciidoc[ici]      | manque page de garde, presentation un peu légère.Reprenez l’installation en expliquant comment lancer l’application. Les screenshots du DL de la JRE n’amène pas de valeur. indiquez les credentials de connexion. Ne manque t il pas des initiales du developpeur en charge pour certaines fonctionalités ? Etoffez vos screenshots avec plus d’exempleset n’oubliez pas de cas d’erreur.
GPO: Le GIT ne devrait pas être structuré par semaine calendaire, mais par version de l’application. Bien que les issues du KanBan soient pas taggés, il semble qu’il y en ai peu par rapport a votre GANTT. Il n’y a rien dans la colonne “to do”, est ce normal ? cela ne semble pas être maintenu à jour. La répartition des taches dans le readme est incomplète.
...
|         | Gantt V2 prévu |  link:/S2-05_Gestion_Projet/GantReelV1-PrevV2.pdf[ici] (Telecharger le pdf pour bonne lisibilité)  | le séquencement est meilleur, mais n’oubliez pas les milestone. Le regroupement par semaine dans la liste des taches à gauche n’a pas d’intéret, trouvez un autre axe qui amène plus de valeur.
